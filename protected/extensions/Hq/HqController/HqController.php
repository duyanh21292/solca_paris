...
class HqController extends CController
{
	...
	/**
	 * Biến cấu hình Vesion Bootstrap sử dụng
	 * Không cấu hình thì không set lại Alias mà sử dụng Alias cấu hình Config
	 *
	 * @var string 'v1' hoặc 'v2'
	 */
	public $version = null;
	...
	function init()
	{
		...
		if(isset($this->version))
			$this->setAliasBootrapsVesion2($this->version);
	}
	...
	function setAliasBootrapsVesion2($v)
	{
		switch ($v) {
			case 'v1':
				Yii::setPathOfAlias('bootstrap', realpath(dirname(Yii::getPathOfAlias('ext.common.common'))).DIRECTORY_SEPARATOR.'bootstrap');
				return;
		}
		Yii::setPathOfAlias('bootstrap', realpath(dirname(Yii::getPathOfAlias('ext.common.common'))).DIRECTORY_SEPARATOR.'bootstrap_' . $v);
	}
